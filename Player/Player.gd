...
func can_interact():
	...
	if len(held_items) < HOLDING_CAPACITY:
		...
		if tile_resource_maker and (!held_items or held_items[0].item_name == tile_resource_maker[0].resource_name):
			return "grid_harvest_interactable"
		tile_resource_maker = Main.world_layers["resource_makers"][front_pos.y][front_pos.x]
		if tile_resource_maker and (!held_items or held_items[0].item_name == tile_resource_maker[0].resource_name):
